"""Encounter tab (v3.1 Part 4).

Single screen with:
- Top: All-characters strip with click-to-place-left/right
- Middle: Left page | Opponent? button / damage exchange / Exit | Right page
- Bottom: Encounter Bin
"""
...
from PyQt6.QtWidgets import (
    ...
    QFrame, QGroupBox, QGridLayout, QMessageBox, QDialog, QDialogButtonBox,
...
class EncounterCard(QFrame):
    """Slim version of a character sheet used inside an encounter page.

    Wraps a full CharacterSheet for now - the spec calls for a more
    focused subset, but reusing the full sheet keeps feature parity
    while still letting the user see/edit everything during a fight.
    """
    ...
    def __init__(self, state: StateManager, instance: EncounterInstance,
        ...
        # Header strip: name, turn counter, dice + history
        header = QFrame()
        ...
        # Full character sheet for full editing inside the encounter.
        # We pass `locked=True` so the sheet doesn't try to lock itself.
        scroll = QScrollArea()
    ...
    def _refresh(self) -> None:
        # Refresh just the header parts (sheet refreshes itself)
        ...
        # Dice history (last 4, oldest faded)
        parts = []
        for i, d in enumerate(self._instance.dice_history):
            if i == 3:
                parts.append(f"<span style='opacity:0.3; color:#888888;'>{d}</span>")
            else:
                parts.append(f"<b>{d}</b>")
        self._dice_log.setText("  ".join(parts) if parts else "(no rolls)")
        # Turn buttons enabled state
...
class CharacterPickerDialog(QDialog):
    """Pop-up to pick a character from the Global Character List to add
    to the encounter."""

    def __init__(self, state: StateManager, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Add Character to Encounter")
        self.resize(500, 500)
        self._state = state
        self.selected_id: Optional[str] = None

        v = QVBoxLayout(self)
        v.addWidget(QLabel("Pick a character:"))
        ...
        for group, role in (("Party", "party"), ("Mobs", "mob"), ("NPCs", "npc")):
            ...
            for c in visible:
                ...
                # Lock check for unique characters already in the encounter
                lock = ""
...
class ConflictPanel(QGroupBox):
    """The damage-exchange middle panel during conflict mode."""
    ...
    def refresh(self) -> None:
        ...
        if enc is None or not enc.in_conflict_mode:
            return
        # Sync UI state to enc selections
        for side, col in (("left", self._left_col), ("right", self._right_col)):
            ...
            # Sync radio + checkbox without re-triggering refresh
        ...
        # Damage received is what the OPPOSITE side deals
        l_inst = self._instance_for_side("left")
        r_inst = self._instance_for_side("right")
        if l_inst and r_inst:
            r_atk = 0.0 if enc.right_is_receiver_only else \
                self._state._atk_value_for_selection(r_inst.character, enc.right_atk_selection)
            l_atk = 0.0 if enc.left_is_receiver_only else \
                self._state._atk_value_for_selection(l_inst.character, enc.left_atk_selection)
            self._left_col["dmg_recv"].setText(f"Damage received: {r_atk:.1f}")
            self._right_col["dmg_recv"].setText(f"Damage received: {l_atk:.1f}")

        # Commit selections back to encounter state on every refresh
        for side, col in (("left", self._left_col), ("right", self._right_col)):
            for k, rb in col["atk_radios"].items():
                if rb.isChecked():
                    if side == "left":
...
class EncounterTab(QWidget):
    def __init__(self, state: StateManager, parent: QWidget | None = None) -> None:
        ...
        self._add_btn = QPushButton("+ Add Character to Encounter")
        ...
        # All-characters strip (scrollable)
        strip_label = QLabel("Encounter roster:")
        strip_label.setProperty("role", "dim")
        ...
        self._left_empty = QLabel("\n(no character placed on the left)\n")
        ...
        self._middle.setMaximumWidth(340)
        ...
        self._right_empty = QLabel("\n(no character placed on the right)\n")
        ...
        # Encounter bin
        bin_label = QLabel("Encounter Bin:")
        bin_label.setProperty("role", "dim")
        outer.addWidget(bin_label)
        self._bin_widget = QWidget()
        self._bin_widget.setFixedHeight(40)
    ...
    def _on_add(self) -> None:
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted and dlg.selected_id:
            source = self._state.find_character(dlg.selected_id)
            if source is None:
                return
    ...
    def _on_end(self) -> None:
        ...
        reply = QMessageBox.question(
            self, "End Encounter?",
            "End the encounter? This will save changes for unique characters "
            "and convert surviving template instances to new unique characters.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if reply == QMessageBox.StandardButton.Yes:
    ...
    def _on_opponent(self) -> None:
        if not self._state.enter_conflict_mode():
            QMessageBox.information(
                self, "Need two combatants",
                "Place a character on the left and right before starting conflict.")
    ...
    def refresh(self) -> None:
        ...
        while self._strip_layout.count() > 1:
            ...
            if w:
                ...
                w.deleteLater()
        # Clear bin
        while self._bin_layout.count() > 1:
            it = self._bin_layout.takeAt(0)
            w = it.widget() if it else None
            if w:
                w.setParent(None)
                w.deleteLater()
        ...
        while self._middle_layout.count():
            it = self._middle_layout.takeAt(0)
            w = it.widget() if it else None
            if w:
                w.setParent(None)
                w.deleteLater()
        # Clear left/right
        for inner, empty in ((self._left_inner, self._left_empty),
                             (self._right_inner, self._right_empty)):
            while inner.count():
                it = inner.takeAt(0)
                w = it.widget() if it else None
                if w and w is not empty:
                    w.setParent(None)
                    w.deleteLater()
        ...
        # Build the strip (every non-binned instance)
        ...
        if l_inst:
            ...
            self._left_inner.addWidget(self._left_empty)
            self._left_empty.setText(
                "\n(click '← L' in the roster strip to place a character here)\n")
        if r_inst:
            ...
            self._right_inner.addWidget(self._right_empty)
            self._right_empty.setText(
                "\n(click 'R →' in the roster strip to place a character here)\n")

        # Middle column: button or conflict panel
